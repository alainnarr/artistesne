...
it('approved artist can access their dashboard', function () {
...
it('approved artist with draft profile sees the profile setup page', function () {
    $user = User::factory()->artist()->create();
    Artist::factory()->for($user)->create(['artist_name' => 'Draft Artist']);

    $this->actingAs($user);

    visit(route('artist.profile.setup'))
        ->assertNoJavaScriptErrors()
...
it('anonymous visitor is redirected to artist login when accessing the portal', function () {
    visit(route('artist.dashboard'))
...
it('artist can re-submit a change request after admin requested adjustments', function () {
    ...
    ArtistChangeRequest::factory()->create([
        'artist_id' => $artist->id,
        'submitted_by' => $user->id,
        'status' => ApprovalStatus::ChangesRequested->value,
