...
@php
    ...
    // ni les selects (chevron) afin de ne pas chevaucher leurs contrôles.
    $hasGutter = $type !== 'date' && $as !== 'select';
...
<div
    ...
    x-init="filled = ($refs.control?.value ?? '') !== ''"
        ...
        >
            ...
            @else
                <input
                    ...
                    x-on:input="filled = $event.target.value !== ''"
                    {{ $attributes->whereStartsWith('wire:')->merge() }}
                    class="peer h-14 w-full rounded-[2px] bg-transparent px-4 text-base text-brand placeholder-transparent focus:outline-none {{ $type === 'date' ? 'date-input pe-10' : '' }}"
                >
