<div>
    ...
    <x-ds.section variant="cream">
        <div class="mx-auto w-full max-w-[928px] bg-brand-paper px-6 py-12 shadow-[0_4px_16px_rgba(27,62,61,0.04)] sm:px-16 sm:py-16">
            ...
            @else
                <div class="mx-auto">
                    <form wire:submit="submit" class="ds-contact-form flex w-full max-w-[500px] flex-col gap-6">
                        <h2 class="font-serif text-2xl font-bold text-brand">Formulaire de contact</h2>

                        <div class="flex flex-col gap-4">
                            <x-ds.input wire:model.blur="last_name" label="Nom" required :error="$errors->first('last_name')" />
                            <x-ds.input wire:model.blur="first_name" label="Prénom" required :error="$errors->first('first_name')" />
                            <x-ds.input wire:model.blur="email" type="email" label="Adresse email" required :error="$errors->first('email')" />
                            <x-ds.input wire:model.blur="subject" label="Objet" required :error="$errors->first('subject')" />
                            <x-ds.textarea wire:model.blur="message" label="Message" :rows="5" required :error="$errors->first('message')" />
                        </div>
